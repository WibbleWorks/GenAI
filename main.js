...
class AICourse {
    ...
    init() {
        ...
        // T5.3 - Placement test: on first visit (zero completed lessons and
        ...
        // lessons 1-5 (AI Intro, ML Intro, NN Intro, Ethics, Math Refresher).
    ...
    _placementOffered() {
        try { return localStorage.getItem('aiCoursePlacementOffered') === '1'; }
        catch (e) { return false; }
    }
    _markPlacementOffered() {
        try { localStorage.setItem('aiCoursePlacementOffered', '1'); }
            ...
            <div style="background: var(--surface-color); border-radius: 12px; padding: 2rem; max-width: 540px; box-shadow: var(--shadow-lg);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 0.5rem; margin-bottom: 1.25rem;">
                    <label style="padding: 0.75rem; background: var(--surface-light); border-radius: 8px; border: 2px solid var(--ai-blue); cursor: pointer;">
                        ...
                        <div style="font-size: 0.75rem; color: var(--text-muted);">Code-first. Applied ML → LLM Systems → Capstone. Less theory, more shipping.</div>
                    </label>
                    <label style="padding: 0.75rem; background: var(--surface-light); border-radius: 8px; border: 2px solid transparent; cursor: pointer;">
                        ...
                        <div style="font-size: 0.75rem; color: var(--text-muted);">Math-first. Math Refresher → DL → Eval → Research topics. Theory + rigor.</div>
                    </label>
                    <label style="padding: 0.75rem; background: var(--surface-light); border-radius: 8px; border: 2px solid transparent; cursor: pointer;">
    ...
    setLearningPath(path) {
        ...
        const startByPath = {
            builder: 'practical_scikit',     // Lesson 6: scikit-learn
            researcher: 'math_refresher',    // Lesson 5: Math refresher (optional but emphasized)
            leader: 'ai_ethics'              // Lesson 4: Ethics
        };
    ...
    startPlacementTest() {
        const foundationsIds = ['ai_introduction', 'ml_introduction', 'neural_networks_intro', 'ai_ethics', 'math_refresher'];
        const questions = [];
    ...
    scorePlacementTest(score, correct, total, passingScore, foundationsIds) {
        ...
        overlay.querySelector('#placementDone')?.addEventListener('click', () => {
            overlay.remove();
            // On pass jump to Lesson 6 (scikit-learn); on fail start at Lesson 1.
            // Either way the user is now in the course proper.
            const targetId = passed ? 'practical_scikit' : 'ai_introduction';
    ...
    showLesson(lessonId) {
        ...
        // Mount any interactive-lab placeholders in the rendered lesson
