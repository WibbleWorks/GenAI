...
class QuizSystem {
    ...
    startQuiz(quiz) {
        ...
        // T5.7 - Shuffle question order, and shuffle each question's options,
        // tracking the mapping so we can still compute correctness and weak
        // concepts against the original question/option indices.
        const n = quiz.questions.length;
        this.questionOrder = this._shuffle([...Array(n).keys()]);
        ...
        // we map back to original option via optionOrder[i][displayIdx].
        this.userAnswers = new Array(n).fill(null);
    ...
    endQuiz() {
        ...
        // Show results
        if (this.quizContainer) {
            this.quizContainer.innerHTML = this.getResultsHTML(finalScore, correct, incorrect, quizSnapshot);
        }
    ...
    closeReview() {
        ...
        if (this.quizContainer) {
            this.quizContainer.innerHTML = this.getResultsHTML(last.score, last.correct, last.incorrect, last.quiz);
        }
    }
            ...
            <div class="quiz-display">
                ...
                <div class="quiz-options" id="quizOptions">
                    ${this.currentQuiz.questions[this.currentQuestionIndex].options.map((option, index) => {
                        ...
                        return `
                            <div class="quiz-option" data-index="${index}" data-question="${this.currentQuestionIndex}" role="button" tabindex="0" aria-label="Option ${letter}: ${option.text}">
                                <span class="option-letter">${letter}</span>
                                <span class="quiz-option-label">${option.text}</span>
                            </div>
                        `;
    ...
    getResultsHTML(score, correct, incorrect, quiz) {
