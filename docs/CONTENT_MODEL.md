...
## JSON source of truth (P5, current)

- `lessons/<level>/<id>.json` + `lessons/manifest.json` (sha hashes) are the
  source of truth. `loader.js` (loaded before `main.js`) fetches the manifest,
  overlays every matching lesson onto inline `COURSE_DATA` (**JSON wins**),
  and exposes `window.courseDataReady` for boot/tests. Unknown ids
  (e.g. quantum-course) are ignored, never leak into core.
- Pilot: `ai_introduction` ships with **no inline body** — it renders purely
  from JSON. The browser smoke asserts its content is non-empty.
- `scripts/extract-lessons.mjs` writes JSON + manifest; `--check` fails CI on
  hash drift or stale files (no silent pruning). `PILOT_JSON_ONLY` lessons are
  never overwritten from inline.
- Non-developer workflow: edit a JSON file → run extractor `--check` locally →
  open a PR. CI runs manifest-check + schema (core + quantum-course) + loader
  harness + codeblocks + smoke + a11y.
