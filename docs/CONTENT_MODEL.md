...
- Enforce the JSON schema from CI (e.g., `ajv` against `docs/lesson.schema.json`) once lessons are exported to JSON files instead of inline JS.
- Move lesson content into `lessons/<level>/<id>.json` so non-developers can edit without touching JS. The renderer would then load JSON via `fetch()` at boot. Saves the current inline-JS template-string fragility.
