...
await page.evaluate(() => {
    try { localStorage.setItem('aiCoursePlacementOffered', '1'); } catch (e) {}
...
const sampleIds = ['ai_introduction', 'practical_scikit', 'rag_vector_databases'];
...
await page.evaluate(() => {
    document.getElementById('placementOverlay')?.remove();
    window.course.showLesson('ai_introduction');
    window.course.startQuiz('ai_introduction');
});
await page.waitForTimeout(200);
...
await page.evaluate(() => {
    document.getElementById('placementOverlay')?.remove();
    window.course.showLesson('practical_scikit');
});
await page.waitForTimeout(200);
